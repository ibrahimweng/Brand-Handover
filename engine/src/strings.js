...
const EN = {
  ...
  // The generated pattern, and — where the brand brought one of its own — the
  // two columns that let anybody check the claim that ours measures like theirs.
  secGenerated: 'The generated pattern',
  genFrom: 'Not a copy of {file}. A pattern built the way every other one here is built, with parameters chosen so that the two measure the same — which means it recolours, re-scales and regenerates, and a copy would do none of those. Both columns below are read off the two pictures the same way, so the claim is one you can check.',
  genFromMark: 'No pattern was supplied, so this one is generated from the mark: from how fine the drawing is, how much of it curves, and which way it runs.',
  genYours: 'Yours', genOurs: 'Ours',
  genRepeatEvery: 'Repeats every', genRepeatAcross: 'Repeats across', genInk: 'Ink',
  genRunsAt: 'Runs at', genEdgesOver: 'Edges over', genRepeatKind: 'Repeat or tendency',
  genInkThickness: 'Ink is', genSquareness: 'Lined up',
  genEveryWay: 'every way', genSquareToPage: 'square to the page', genOnTheDiagonal: 'on the diagonal',
  genNoRepeat: 'no repeat', genNoDirection: 'no direction',
  genARepeat: 'a repeat', genATendency: 'a tendency', genNone: '—',
  genNotMatched: 'not compared', genBeyond: 'beyond what this engine draws',
  genMissed: 'this one is off',
  genStudio: 'Open pattern-studio.html in this package to change any of it: the same generators, the same parameters, offline, exporting at any size.',
  genAlso: 'Other generators measure the same as this one, so any of them would match: {names}.',
...
const FR = {
  ...
  secGenerated: 'Le motif généré',
  genFrom: 'Ce n’est pas une copie de {file}. C’est un motif construit comme tous les autres ici, avec des paramètres choisis pour que les deux se mesurent pareil — il se recolore, se redimensionne et se régénère, ce qu’une copie ne ferait pas. Les deux colonnes ci-dessous sont lues sur les deux images de la même façon, afin que l’affirmation soit vérifiable.',
  genFromMark: 'Aucun motif n’a été fourni ; celui-ci est donc généré à partir du symbole : de la finesse du dessin, de la part qui est courbe, et du sens dans lequel il court.',
  genYours: 'Le vôtre', genOurs: 'Le nôtre',
  genRepeatEvery: 'Se répète tous les', genRepeatAcross: 'Répétitions en largeur', genInk: 'Encre',
  genRunsAt: 'Orientation', genEdgesOver: 'Contours sur', genRepeatKind: 'Répétition ou tendance',
  genInkThickness: 'Épaisseur de l’encre', genSquareness: 'Aligné',
  genEveryWay: 'dans tous les sens', genSquareToPage: 'sur les axes de la page', genOnTheDiagonal: 'en diagonale',
  genNoRepeat: 'aucune répétition', genNoDirection: 'aucune direction',
  genARepeat: 'une répétition', genATendency: 'une tendance', genNone: '—',
  genNotMatched: 'non comparé', genBeyond: 'hors de ce que ce moteur dessine',
  genMissed: 'celle-ci est à côté',
  genStudio: 'Ouvrez pattern-studio.html dans ce dossier pour tout modifier : les mêmes générateurs, les mêmes paramètres, hors ligne, exportables à toute taille.',
  genAlso: 'D’autres générateurs se mesurent comme celui-ci ; n’importe lequel conviendrait : {names}.',
...
Object.assign(EN, {
  ...
  whyOnly: 'The only shape in the drawing',
...
Object.assign(FR, {
  ...
  whyOnly: 'La seule forme du dessin',
...
cvArtLockup: 'lockup', cvArtMarkOpt: 'mark', cvArtPatternOpt: 'pattern',
  ...
  cvBlockGenerated: 'Generated pattern',
  cvNoGenerated: 'No generated pattern in this package.',
  cvArtGenerated: 'The {generator} pattern, {style}',
  cvGeneratedRule: '{generator} · {style} — generated, and the same on every page',
  cvRetouch: 'The pattern', cvRetouchAll: 'Changing this changes every generated pattern in the document — it is one decision, not a picture.',
  cvRetouchBack: 'Back to what the engine chose',
  ...
  cvMotionOneStroke: '{out} · out · one piece, no separate fill to rise',
...
Object.assign(FR, {
  ...
  cvBlockGenerated: 'Motif généré',
  cvNoGenerated: 'Aucun motif généré dans ce dossier.',
  cvArtGenerated: 'Le motif {generator}, {style}',
  cvGeneratedRule: '{generator} · {style} — généré, et identique sur chaque page',
  cvRetouch: 'Le motif', cvRetouchAll: 'Le modifier ici le modifie sur toutes les pages du document : c’est une décision, pas une image.',
  cvRetouchBack: 'Revenir à ce que le moteur a choisi',
  ...
  cvMotionOneStroke: "{out} · sortie · une seule pièce, pas d'aplat distinct à faire monter",
...
const HE = {
  ...
  secScale: "הסולם",
  secPattern: "הדוגמה",
  secGenerated: "הדוגמה שנוצרה",
  cvBlockGenerated: "דוגמה שנוצרה",
  cvNoGenerated: "אין דוגמה שנוצרה בחבילה הזו.",
  cvArtGenerated: "דוגמת {generator}, {style}",
  cvGeneratedRule: "{generator} · {style} — נוצרה, וזהה בכל עמוד",
  cvRetouch: "הדוגמה", cvRetouchAll: "שינוי כאן משנה כל דוגמה שנוצרה במסמך — זו החלטה אחת, לא תמונה.",
  cvRetouchBack: "חזרה למה שהמנוע בחר",
  genFrom: "זו אינה העתקה של {file}. זו דוגמה שנבנתה כמו כל האחרות כאן, עם פרמטרים שנבחרו כך ששתיהן יימדדו אותו הדבר — היא ניתנת לצביעה מחדש, להגדלה ולהפקה מחדש, מה שהעתקה אינה מאפשרת. שני הטורים שלהלן נמדדו משתי התמונות באותה הדרך, כך שאפשר לבדוק את הטענה.",
  genFromMark: "לא סופקה דוגמה, ולכן זו נוצרה מתוך הסמל: מרמת העדינות של השרטוט, מהחלק שבו הוא מעוגל, ומהכיוון שבו הוא רץ.",
  genYours: "שלכם", genOurs: "שלנו",
  genRepeatEvery: "חוזרת כל", genRepeatAcross: "חזרות לרוחב", genInk: "דיו",
  genRunsAt: "כיוון", genEdgesOver: "מעבר על פני", genRepeatKind: "חזרה או נטייה",
  genInkThickness: "עובי הדיו", genSquareness: "מיושר",
  genEveryWay: "לכל הכיוונים", genSquareToPage: "לצירי העמוד", genOnTheDiagonal: "באלכסון",
  genNoRepeat: "אין חזרה", genNoDirection: "אין כיוון",
  genARepeat: "חזרה", genATendency: "נטייה", genNone: "—",
  genNotMatched: "לא הושווה", genBeyond: "מעבר למה שהמנוע הזה מצייר",
  genMissed: "זו לא תואמת",
  genStudio: "פתחו את pattern-studio.html שבחבילה כדי לשנות כל חלק ממנה: אותם מחוללים, אותם פרמטרים, בלי חיבור לרשת, וייצוא בכל גודל.",
  genAlso: "מחוללים אחרים נמדדים כמו זה, ולכן כל אחד מהם היה מתאים: {names}.",
...
const JA = {
  ...
  secTypefaces: "書体",
  secPattern: "パターン",
  // Written inside what 山彦's own subset can draw. That font ships with the
  // package so it opens with no network, and a subset holds what somebody knew
  // about when it was cut: thirteen characters of the first draft of these
  // lines were outside it, and test/run.js said so. ネットワーク is one of the
  // casualties — the subset has no ワ — so the line says 回線 instead.
  secGenerated: "生成したパターン",
  cvBlockGenerated: "生成したパターン",
  cvNoGenerated: "この一式に生成したパターンはありません。",
  cvArtGenerated: "{generator} のパターン、{style}",
  cvGeneratedRule: "{generator} · {style} — 生成したもの。どの頁でも同じです",
  cvRetouch: "パターン", cvRetouchAll: "ここで変えると、この書き物にある生成したパターンが全て変わります。一つの決めごとであって、画像ではありません。",
  cvRetouchBack: "エンジンが選んだものに戻す",
  genFrom: "{file} を写したものではありません。ここにある他のパターンと同じやり方で作り、両方が同じ数値になるようにパラメータを選んでいます。だからこそ色を変え、大きさを変え、作り直すことができます。写したものにはできないことです。下の二列はどちらも同じやり方で二枚の画像から測っているので、そのまま確かめられます。",
  genFromMark: "パターンが無かったため、これはマークから生成しています。線の細かさ、輪郭のうち曲線が占める割合、そして向きによるものです。",
  genYours: "既にあるもの", genOurs: "生成したもの",
  genRepeatEvery: "繰り返しの間隔", genRepeatAcross: "横方向の繰り返し数", genInk: "墨の割合",
  genRunsAt: "向き", genEdgesOver: "輪郭の幅", genRepeatKind: "繰り返しか、ゆるやかか",
  genInkThickness: "墨の太さ", genSquareness: "そろい方",
  genEveryWay: "どの向きにも", genSquareToPage: "頁の縦横に", genOnTheDiagonal: "ななめに",
  genNoRepeat: "繰り返し無し", genNoDirection: "向き無し",
  genARepeat: "繰り返し", genATendency: "ゆるやか", genNone: "—",
  genNotMatched: "比べていません", genBeyond: "このエンジンが描ける範囲の外です",
  genMissed: "これは合っていません",
  genStudio: "この一式に入っている pattern-studio.html を開けば、どこでも変えられます。同じ生成の仕組み、同じパラメータ、回線に つながずに、どの大きさにも書き出せます。",
  genAlso: "他の生成の仕組みもこれと同じ数値になるため、どれを選んでも合います: {names}。",
  secMotion: "動き",
  secMadeAs: "どうつくられるか",
...
const CANVAS_SHARED = ['badgeSystem', 'badgeOnce', 'badgeYours',
  ...
  'secMotion', 'secPhotography', 'secConstruction', 'secTypefaces', 'secGenerated',
