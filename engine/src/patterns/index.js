...
const GENERATORS = {
  weave: require('./generators/weave'),
  zigzag: require('./generators/zigzag'),
};
...
function tile(opts) {
  const o = opts || {};
  const m = o.mark || markRead.read(o.markSource, o.measured, o.rules);
...
const joins = (t, opts) => seam.check(t.paint, opts && opts.size ? opts.size : 100,
  opts && opts.size ? opts.size : 100, opts);

module.exports = { GENERATORS, NAMES, suits, derive, because, tile, joins,
  read: markRead.read, FINEST, COARSEST_STRIPE, FINEST_STRIPE, WEAVE_STYLES, ZIGZAG_STYLES };
