...
.mtrack i{position:absolute;top:0;height:9px;display:block}
.mrow em{font-family:var(--mono);font-style:normal;font-size:10.5px;color:var(--ink-3);text-align:right}
...
.ftr.head span:nth-child(3),.ftr.head span:nth-child(4){text-align:right}
...
function displayVar(style, type) {
  ...
  return `<style>:root{--display:${JSON.stringify(fam.family)}, ${fam.fallback || 'Helvetica, Arial, sans-serif'}, var(--pkg,sans-serif), sans-serif}</style>`;
}
